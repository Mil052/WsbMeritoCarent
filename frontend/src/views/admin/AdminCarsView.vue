...
<template>
  <div class="flex justify-between items-center mx-4 sm:mx-8 lg:mx-16 my-16">
    <h1 class="text-xl xs:text-2xl">Samochody</h1>
    <RouterLink :to="{name: 'admin-add-car'}" class="flex gap-4 items-center">
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M10 1V19M1 10C1.4 10 13.1667 10 19 10" stroke-width="1.5" stroke-linecap="round" class="stroke-black dark:stroke-white"/>
      </svg>
      <span>dodaj nowy</span>
    </RouterLink>
  </div>
  <!-- Car Filters -->
  <div class="mx-4 sm:mx-8 my-16">
    <form @submit.prevent="filterCars">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 lg:gap-16">
        <div class="w-full">
          <h3 class="text-sm text-neutral-600 dark:text-neutral-300 my-2 md:my-4">MARKA</h3>
          <CarMakerSelect v-model:maker="maker" />
        </div>
        <div class="w-full">
          <h3 class="text-sm text-neutral-600 dark:text-neutral-300 my-2 md:my-4">RODZAJ PALIWA</h3>
          <FuelTypeSelect v-model:fuel="fuel" />
        </div>
        <div class="w-full">
          <h3 class="text-sm text-neutral-600 dark:text-neutral-300 my-2 md:my-4">CENA</h3>
  ...
  <section class="mx-4 sm:mx-8 my-16 p-4 xl:p-8 bg-light-bg rounded-lg text-black">
    <div class="flex gap-x-8 gap-y-4 flex-wrap justify-between my-8 mx-4 sm:mx-8">
      <h2 class="text-xl xs:text-2xl">Parametry samochodu</h2>
      <div v-if="selectedCar" class="flex gap-8">
        <RouterLink :to="{name: 'admin-edit-car', params: {id: selectedCar._id}}" class="flex gap-3 btn-secondary">
          <PenSvg />
          <span>Edytuj</span>
        </RouterLink>
    ...
    <div v-if="selectedCar" class="flex flex-col md:flex-row gap-4 md:gap-8">
      <div class="min-w-2xs max-w-lg w-full mx-auto md:w-2/5">
        <CarDetailsCard :car="selectedCar"/>
      </div>
      <div class="min-w-2xs md:w-3/5">
